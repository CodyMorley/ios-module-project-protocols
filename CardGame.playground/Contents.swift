    ...
    init() {
        ...
        for rank in CardRank.allRanks {
            for suit in CardSuit.allSuits {
                newCard.append(Card(rank: rank, suit: suit))
            }
        }
        self.contents = newCard
    }
    
    func drawCard() -> Card {
        let randomNumber: Int = Int.random(in: 0...contents.count)
...
extension CardRank: CaseIterable {
    static var allRanks: [CardRank] {
        return CardRank.allCases
    }
}
...
class HighLow: CardGame, CardGameDelegate {
    ...
    func play() {
        ...
        } else if drawPlayer1 < drawPlayer2 {
            print("Player 2 wins with a \(drawPlayer2.description)!")
        } else {
            print("Player 1 wins with a \(drawPlayer1.description)!")
        }
    }
